...
import { FilterTabs, type CategoryFilter, type SortMode } from '../components/market/FilterTabs'
...
function sortCoins(coins: Coin[], mode: SortMode): Coin[] {
  ...
  switch (mode) {
    case 'trending':
      return list.sort((a, b) => b.volume24h / b.marketCap - a.volume24h / a.marketCap)
    case 'gainersLosers':
...
export function MarketPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter((c) => `${c.name} ${c.symbol}`.toLowerCase().includes(q))
    }
  ...
  return (
    <main className="mx-auto max-w-310">
      ...
      <section className="px-5 pb-21 pt-2.5 nav:px-7.5">
        ...
        <Pagination page={page} totalPages={totalPages} onPage={setPage} rows={rows} onRows={updateRows} />
      </section>
    </main>
  )
}
