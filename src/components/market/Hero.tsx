  ...
  return (
    <section className="grid gap-10 px-5 py-11 nav:grid-cols-[1fr_1.1fr] nav:items-center nav:gap-15 nav:px-7.5 nav:py-14">
      <div>
        ...
        <p className="my-5.5 max-w-95 text-sm leading-[1.8] text-muted">{t.hero.copy}</p>
        <div className="flex flex-wrap gap-7.5">
          <div className="flex flex-col gap-0.5">
            <strong className="font-display text-lg font-bold">{formatCompactUsd(globalStats.totalMarketCap, lang)}</strong>
            <span className="text-[10px] text-muted">{t.hero.marketCapLabel}</span>
          </div>
          <div className="flex flex-col gap-0.5">
            <strong className="font-display text-lg font-bold text-green">{formatPercent(globalStats.marketCapChange24h, lang)}</strong>
            <span className="text-[10px] text-muted">{t.hero.changeLabel}</span>
          </div>
          <div className="flex flex-col gap-0.5">
            <strong className="font-display text-lg font-bold">{formatCompactUsd(globalStats.totalVolume24h, lang)}</strong>
